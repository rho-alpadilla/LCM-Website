...
export function PrayerContactReveal({ requestId }: { requestId: string }) {
  ...
  async function reveal() {
    ...
    setError("");
    try {
      const response = await fetch(`/api/admin/prayer/${requestId}/contact`, {
        cache: "no-store",
      });
      if (!response.ok)
        throw new Error("Contact details could not be revealed.");
      const result = (await response.json()) as { contact: Contact | null };
