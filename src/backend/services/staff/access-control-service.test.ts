...
function createRepositoryStub(
  ...
  return {
    ...
    findReadyInvitationByEmail: vi.fn().mockResolvedValue(null),
    findPendingInvitationById: vi.fn().mockResolvedValue(null),
    createInvitation: vi.fn().mockResolvedValue(undefined),
    updateInvitationProvisioning: vi.fn().mockResolvedValue(undefined),
...
describe("AccessControlService", () => {
  ...
  it("activates only an invitation matching the verified email", async () => {
    const invitation: PendingInvitation = {
      ...
      accessProvisioningStatus: "ready",
      accessProvisioningErrorCode: null,
      accessProvisioningAttempts: 1,
      accessProvisioningUpdatedAt: "2026-09-10T00:00:00.000Z",
      createdAt: "2026-09-10T00:00:00.000Z",
    };
    ...
    const service = new AccessControlService(
      createRepositoryStub({
        findReadyInvitationByEmail: vi.fn().mockResolvedValue(invitation),
        activateInvitation,
      }),
      {
        createId: () => ids.shift() ?? "unexpected-id",
  ...
  it("does not activate an invitation until secure Access setup is ready", async () => {
  ...
  it("records an Access setup issue without discarding the staff invitation", async () => {
    const invitation: PendingInvitation = {
      ...
      invitedBy: "5d3a2ee4-7f94-4e95-ae5b-5c0650b8749e",
      accessProvisioningStatus: "setting_up",
      accessProvisioningErrorCode: null,
      accessProvisioningAttempts: 0,
      accessProvisioningUpdatedAt: null,
      createdAt: "2026-09-10T00:00:00.000Z",
    };
    const updateInvitationProvisioning = vi.fn().mockResolvedValue(undefined);
    const service = new AccessControlService(
      createRepositoryStub({
      ...
      {
        createId: () => "audit-or-correlation-id",
    ...
    await service.recordInvitationProvisioning({
      ...
      failureCode: "staff_access_update_failed",
    });

    expect(updateInvitationProvisioning).toHaveBeenCalledWith(
      expect.objectContaining({
        invitationId: invitation.id,
        status: "needs_attention",
        failureCode: "staff_access_update_failed",
