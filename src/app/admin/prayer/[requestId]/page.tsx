...
import { PrayerContactReveal } from "@/components/admin/prayer-contact-reveal";
import { requireActiveStaffSession } from "@/features/auth/staff-context";
import {
  addPrayerUpdateAction,
  assignPrayerAction,
  closePrayerAction,
} from "@/features/prayer/actions";
...
export default async function PrayerDetailPage({
  ...
  const [detail, assignees] = await Promise.all([
    service.getDetail(state.context, requestId),
    state.context.permissions.includes("prayer.assign")
      ? service.listEligibleAssignees(state.context)
      : Promise.resolve([]),
  ]);
